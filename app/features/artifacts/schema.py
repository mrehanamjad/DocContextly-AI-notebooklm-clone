...
#     length: Literal["short", "medium", "long"] = Field("medium", description="Target length of the slide deck") 
...
class ArtifactShortResponse(BaseModel):
    ...
    status: ArtifactStatus
    title: str
    # Included so the frontend's version-history dropdown (ArtifactHeader) can
    # show correct generation options (length, voice_style, prompt, etc.)
    # without needing a full detail fetch for every historical entry.
