...
class SourceService:
    ...
    async def create_website_sources(
        ...
        """
        Accepts a list of website URLs (1 to 20).
        Concurrently fetches HTML, handles duplication rules, bulk saves, 
        and triggers background indexing tasks.
        """
        # 1. Clean and validate the URL list
        valid_urls = [u.strip() for u in urls if u and u.strip()]
        
        if not valid_urls:
            raise BadRequestException("No valid URLs provided for ingestion.")

        # Enforce the 20 URL limit safely on the cleaned list
        if len(valid_urls) > 20:
            raise BadRequestException("You can only process up to 20 URLs at a time.")

        await self.notebook_service.get_notebook(notebook_id, user_id)

        # 2. Concurrently fetch all websites
        web_docs = await load_web(valid_urls)
        
        if not web_docs:
            raise BadRequestException("Failed to retrieve content from the provided websites.")

        all_new_sources = []
        responses = []

        # 3. Process each scraped document
        for doc in web_docs:
            ...
            clean_url = url.strip()
            source_id = hashlib.md5(clean_url.encode("utf-8", errors="ignore")).hexdigest()

            try:
                # Rule 1 (Scope) and Rule 2 (Cross-Notebook) Validation
                linked_source = await self._handle_duplication(source_id, user_id, notebook_id)
            ...
            # Brand new source preparation with accurate word count
            source_data = {
                "url": clean_url,
                "title": title,
                "content": content,
        ...
        # 4. Bulk Insert and Dispatch Background Workers
        if all_new_sources:
            ...
            for source in inserted_sources:
                ...
                # Attach the safe callback to catch silent crashes
