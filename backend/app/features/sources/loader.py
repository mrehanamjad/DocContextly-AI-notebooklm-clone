...
async def load_web(urls: list[str]) -> list[Document]:
    ...
    try:
        ...
        if not raw_html_docs:
            logger.warning("No HTML content could be successfully retrieved.")
            return []

        transformer = Html2TextTransformer()
        
        logger.info("Transforming raw HTML payloads into structured Markdown...")
        # FIXED: Actually offloading the CPU-bound task to a thread pool
        cleaned_markdown_docs = await asyncio.to_thread(
            transformer.transform_documents, raw_html_docs
        )
        
        return cleaned_markdown_docs

    except Exception as e:
        logger.error(f"Critical failure in web ingestion pipeline: {e}", exc_info=True)
        raise InternalServerError(f"Failed to process external web targets: {e}")

async def get_topic_urls(topic: str,max_res:int) -> list[str]:
    """Searches the web for a topic and returns a list of target URLs."""
    ...
    try:
        ...
        if not urls:
            logger.warning(f"Tavily returned zero valid target URLs for: '{clean_topic}'")
            return []
            
        return urls

    except Exception as e:
        logger.error(f"Failed search workflow for topic '{clean_topic}': {e}", exc_info=True)
        raise InternalServerError(f"Failed to orchestrate topic research pipeline: {e}")
